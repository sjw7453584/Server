...
// no DB_TYPE_BOOLEAN for now, it will be a 1/0 integer
        ...
        Field();
        Field(Field &f);
        Field(const char *value, int valueSize, const char *name, enum DataTypes type);
		...
		int GetValueSize() const { return mValueSize; }
        float GetFloat() const { return mValue ? static_cast<float>(atof(mValue)) : 0; }
		double GetDouble() const { return mValue ? static_cast<double>(atof(mValue)) : 0; }
        ...
        void SetValue(const char *value, int valueSize);
